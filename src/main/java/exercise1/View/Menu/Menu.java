package exercise1.View.Menu;

import exercise1.model.DBLayer.MongoDBRepository;
import exercise1.model.FigureHandler;
...
public class Menu {
    ...
    private static MongoDBRepository mdbr;
...
//        db.establishDefaultConnection();
        ...
        in = new Scanner(System.in);
        String pass = in.nextLine();
//        mdbr = new MongoDBRepository(pass);
    ...
    private static void menu(boolean isShown) {
        ...
        while (isShown) {
            System.out.println("1 - Вывести список всех фигур\n" +
                    ...
                    "6 - Выход\n" +
                    "7 - Переписать все фигуры из файла в БД\n" +
                    "8 - Показать все фигуры из БД\n");
            ...
            switch (mainMenuNum) {
                ...
                case 7: {
                    for (Shape shape : fh.getFigureList()) {
                        mdbr.insert(shape);
                    }
                    break;
                }
                case 8: {
                    List <Shape> asdf = mdbr.findAll();
                    for(Shape shape : asdf) {
                        System.out.println(shape.getRuName());
                    }
                    break;
                }
                default: {
...
//            FigureMenu.handle(fh.getFigureList().get(figureNum - 1));
            List<Shape> shapes = mdbr.findById(figureNum);
    ...
    private static void choiceToRemove() {
        ...
        int figureNum = in.nextInt();
        if (figureNum <= 4 && figureNum >= 1) {
//            fh.getFigureList().remove(figureNum - 1);
            mdbr.deleteById(figureNum);
        } else {
            System.out.println("Некорректный ввод");
            choiceToRemove();
        }
        ...
        public static void handle(Shape figure) {
            if (figure.getRuName().equalsIgnoreCase(ShapeTypes.CIRCLE.getRuName())) {
                ...
                System.out.println("Выбранная фигура: " + figure.getRuName() +
                        " - с координатами в точках - " + Utils.getPointsAsString(figure.getPoints()));
            }
